...
class ReviewsAdapter(val context: Context, val restaurant: Restaurant, val reviews: List<Review>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if(position == 0)
            (holder as HeaderViewHolder).bind(restaurant)
        else
